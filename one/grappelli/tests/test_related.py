...
class RelatedTests(TestCase):
    ...
    def test_related_lookup(self):
        ...
        response = self.client.get(
            "{}?object_id=1&app_label={}&model_name={}".format(reverse("grp_related_lookup"), "tests", "entry")
        )
        self.assertEqual(response.status_code, 200)
        self.assertJSONEqual(
            response.content.decode("utf-8"), [{"value": "1", "label": "Entry Superuser", "safe": False}]
        )
        response = self.client.get(
            "{}?object_id=2&app_label={}&model_name={}".format(reverse("grp_related_lookup"), "tests", "entry")
        )
        self.assertEqual(response.status_code, 200)
        self.assertJSONEqual(
            response.content.decode("utf-8"), [{"value": "2", "label": "Entry Editor", "safe": False}]
        )
