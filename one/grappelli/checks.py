...
def register_checks():
    register(Tags.models)(check_models)  # noqa
...
def check_models(app_configs, **kwargs):
    ...
    for app_config in app_configs:
        for model in app_config.get_models():
            errors.extend(check_model(model))  # noqa
    return errors
...
def _check_autocomplete_search_fields(model):
    ...
    for lookup in model.autocomplete_search_fields():
        try:
            ...
            model._default_manager.filter(**{lookup: F("pk")})  # noqa
        except FieldError:
        ...
        return [
            Error(
                ...
                "autocomplete_search_fields: {failures}".format(
                    app=model._meta.app_label,  # noqa
