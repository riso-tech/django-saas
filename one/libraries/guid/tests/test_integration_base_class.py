...
def test_missing_run_method(monkeypatch, client):
    ...
    with override_settings(DJANGO_GUID=mocked_settings):
        ...
        with pytest.raises(
            ImproperlyConfigured, match="The integration `SentryIntegration` is missing a `run` method"
        ):
            client.get("/api")


def test_run_method_not_accepting_kwargs():
    """
...
def test_cleanup_method_not_accepting_kwargs():
    """
    Tests that an exception is raised when the run method doesn't accept kwargs.
    """
    from one.libraries.guid.config import Settings
