...
(function ($) {
  var methods = {
    init: function (options) {
      ...
      return this.each(function () {
        ...
        var val =
          $(options.content_type).val() ||
          $(options.content_type).find(':checked').val();
        if (val) {
          $this
            .after(loader)
            .after(remove_link($this.attr('id')))
            .after(lookup_link($this.attr('id'), val));
        }
        $this
          .parent()
        ...
        $(options.content_type).on('change', function () {
  ...
  $.fn.grp_autocomplete_generic = function (method) {
    if (methods[method]) {
      ...
      $.error(
        'Method ' +
          method +
          ' does not exist on jQuery.grp_autocomplete_generic',
      );
    }
    return false;
  };
  ...
  var lookup_link = function (id, val) {
    ...
    lookuplink.attr(
      'href',
      window.ADMIN_URL +
        MODEL_URL_ARRAY[val].app +
        '/' +
        MODEL_URL_ARRAY[val].model +
        '/?',
    );
  ...
  var update_lookup = function (elem, options) {
    ...
    if (val) {
      obj
        .after(loader)
        .after(remove_link(obj.attr('id')))
        .after(lookup_link(obj.attr('id'), val));
        ...
        source: function (request, response) {
          $.ajax({
            ...
            data:
              ...
              '&app_label=' +
              grappelli.get_app_label(elem) +
              '&model_name=' +
              grappelli.get_model_name(elem) +
              '&query_string=' +
              grappelli.get_query_string(elem),
