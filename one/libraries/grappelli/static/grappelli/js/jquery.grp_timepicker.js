...
(function ($) {
  $.widget('ui.grp_timepicker', {
    ...
    options: {
      ...
      template:
        '<div id="ui-timepicker" class="module" style="position: absolute; display: none;"></div>',
    ...
    _create: function () {
      ...
      $(document).on('mousedown', function (evt) {
        if (self.timepicker.is(':visible')) {
          ...
          if (
            $target[0].id != self.timepicker[0].id &&
            $target.parents(self.options.timepicker_selector).length === 0 &&
            !$target.hasClass('hasTimepicker') &&
            !$target.hasClass('ui-timepicker-trigger')
          ) {
