...
(function ($) {
  ...
  $.fn.grp_related_generic = function (method) {
    if (methods[method]) {
      ...
      $.error(
        'Method ' + method + ' does not exist on jQuery.grp_related_generic',
      );
    }
    return false;
  };
  ...
  var lookup_id = function (elem, options) {
    ...
    $.getJSON(
      ...
      function (data) {
        ...
        if (data[0].safe) {
          text.html(
            $('<span class="grp-placeholder-label"></span>').html(
              data[0].label + '\u200E',
            ),
          );
        } else {
          text.html(
            $('<span class="grp-placeholder-label"></span>').text(
              data[0].label + '\u200E',
            ),
          );
        }
  ...
  $.fn.grp_related_generic.defaults = {
    placeholder:
      '<div class="grp-placeholder-related-generic" style="display:none"></div>',
    repr_max_length: 30,
    lookup_url: '',
    content_type: '',
    object_id: '',
