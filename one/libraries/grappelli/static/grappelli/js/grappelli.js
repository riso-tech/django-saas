    ...
    if (method === 'confirm') {
      ...
      if (windowQuery[0] !== undefined && windowQuery[0] !== '') {
        windowQuery.map((param) => {
          ...
          var fieldNameIndex = windowQueryDict.findIndex(
          ...
          if (fieldNameIndex === -1) {
            ...
            windowQueryDict
              .find((obj) => obj.fieldName === fieldName)
              .values.push(param);
          }
        });
      }
      ...
      $('.grp-filter-choice').change(function () {
        ...
        var value =
          $(this).val() !== '?' ? $(this).val().replace('?', '') : false;
        var values = value && value.split('&');
        var filterQueryParams =
          values && values.filter((el) => el.includes(fieldName));
        // Check if fieldName already exists in filterQueryDict and add it resp. its values
        var filterWindowIndex = windowQueryDict.findIndex(
          (el) => el.fieldName === fieldName,
        );
        ...
        if (isFilterPartOfWindow) {
          if (filterQueryParams.length > 0) {
            // Update query params
            windowQueryDict.find((el) => el.fieldName === fieldName).values =
              filterQueryParams;
          } else {
            // Remove filter
