...
from ..settings import ADMIN_TITLE, ADMIN_URL, CLEAN_INPUT_TYPES, SWITCH_USER, SWITCH_USER_ORIGINAL, SWITCH_USER_TARGET
...
def switch_user_dropdown(context):
    if SWITCH_USER:
        ...
        session_user = request.session.get(
            "original_user",
            {"id": request.user.id, "username": request.user.get_username(), "name": request.user.name},
        )
        try:
        ...
        if SWITCH_USER_ORIGINAL(original_user):
            object_list = [
                user
                for user in User.objects.filter(is_staff=True).exclude(pk=original_user.pk)
