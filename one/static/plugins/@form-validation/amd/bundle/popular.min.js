        ...
        return (
          ...
          (i.prototype.onElementIgnored = function (e) {
            ...
            if (this.messages.has(i)) {
              var r = this.messages.get(i);
              [].slice
                .call(r.querySelectorAll('[data-field="'.concat(e.field, '"]')))
                .forEach(function (e) {
                  r.removeChild(e);
                });
            }
        ...
        return {
          validate: function (r) {
            ...
            var x = parseInt(g, 10),
              ...
              V = parseInt(b, 10);
            if (!n(x, E, V)) return s;
            var w = new Date(x, E - 1, V);
            if (d) {
              var A = p.split(':');
              if (d.split(':').length !== A.length) return s;
              var O =
                  A.length > 0
                ...
                C =
                  A.length > 2
                    ? A[2].length <= 2 && /^\d+$/.test(A[2])
                      ...
                      : -1
                    : 0;
              if (-1 === O || -1 === F || -1 === C) return s;
              if (C < 0 || C > 60) return s;
              if (O < 0 || O >= 24 || (c && O > 12)) return s;
              if (F < 0 || F > 59) return s;
              w.setHours(O), w.setMinutes(F), w.setSeconds(C);
            }
            var I = 'function' == typeof o.min ? o.min() : o.min,
              H = I instanceof Date ? I : I ? e(I, v, h) : w,
              _ = 'function' == typeof o.max ? o.max() : o.max,
              S = _ instanceof Date ? _ : _ ? e(_, v, h) : w,
              k = I instanceof Date ? i(H, o.format) : I,
              j = _ instanceof Date ? i(S, o.format) : _;
            switch (!0) {
              case !!k && !j:
                return {
                  message: t(r.l10n ? r.l10n.date.min : a, k),
                  meta: { date: w },
                  valid: w.getTime() >= H.getTime(),
                };
              case !!j && !k:
                return {
                  message: t(r.l10n ? r.l10n.date.max : a, j),
                  meta: { date: w },
                  valid: w.getTime() <= S.getTime(),
                };
              case !!j && !!k:
                return {
                  message: t(r.l10n ? r.l10n.date.range : a, [k, j]),
                  meta: { date: w },
                  valid:
                    w.getTime() <= S.getTime() && w.getTime() >= H.getTime(),
                };
              default:
                return { message: ''.concat(a), meta: { date: w }, valid: !0 };
            }
          },
        };
      }),
          ...
          validate: function (t) {
            ...
            var r = n.min ? ''.concat(n.min) : '',
              ...
              a = n.utf8Bytes
                ...
                : i.length,
              s = !0,
              l = t.l10n ? n.message || t.l10n.stringLength.default : n.message;
            switch (
