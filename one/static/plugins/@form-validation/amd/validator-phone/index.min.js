/**
 * FormValidation (https://formvalidation.io)
 * The best validation library for JavaScript
 * (c) 2013 - 2023 Nguyen Huu Phuoc <me@phuoc.ng>
 *
 * @license https://formvalidation.io/license
 * @package @form-validation/validator-phone
 * @version 2.0.2
 */

define(['exports', '@form-validation/core'], function (s, e) {
  'use strict';
  var t = e.utils.format;
  s.phone = function () {
    var s = [
      'AE',
      'BG',
      'BR',
      'CN',
      'CZ',
      'DE',
      'DK',
      'ES',
      'FR',
      'GB',
      'IN',
      'MA',
      'NL',
      'PK',
      'RO',
      'RU',
      'SK',
      'TH',
      'US',
      'VE',
    ];
    return {
      validate: function (e) {
        if ('' === e.value) return { valid: !0 };
        var a = Object.assign({}, { message: '' }, e.options),
          d = e.value.trim(),
          r = d.substr(0, 2);
        if (
          !(r =
            'function' == typeof a.country
              ? a.country.call(this)
              : a.country) ||
          -1 === s.indexOf(r.toUpperCase())
        )
          return { valid: !0 };
        var n = !0;
        switch (r.toUpperCase()) {
          case 'AE':
            n =
              /^(((\+|00)?971[\s.-]?(\(0\)[\s.-]?)?|0)(\(5(0|2|5|6)\)|5(0|2|5|6)|2|3|4|6|7|9)|60)([\s.-]?[0-9]){7}$/.test(
                d,
              );
            break;
          case 'BG':
            n =
              /^(0|359|00)(((700|900)[0-9]{5}|((800)[0-9]{5}|(800)[0-9]{4}))|(87|88|89)([0-9]{7})|((2[0-9]{7})|(([3-9][0-9])(([0-9]{6})|([0-9]{5})))))$/.test(
                d.replace(/\+|\s|-|\/|\(|\)/gi, ''),
              );
            break;
          case 'BR':
            n =
              /^(([\d]{4}[-.\s]{1}[\d]{2,3}[-.\s]{1}[\d]{2}[-.\s]{1}[\d]{2})|([\d]{4}[-.\s]{1}[\d]{3}[-.\s]{1}[\d]{4})|((\(?\+?[0-9]{2}\)?\s?)?(\(?\d{2}\)?\s?)?\d{4,5}[-.\s]?\d{4}))$/.test(
                d,
              );
            break;
          case 'CN':
            n =
              /^((00|\+)?(86(?:-| )))?((\d{11})|(\d{3}[- ]{1}\d{4}[- ]{1}\d{4})|((\d{2,4}[- ]){1}(\d{7,8}|(\d{3,4}[- ]{1}\d{4}))([- ]{1}\d{1,4})?))$/.test(
                d,
              );
            break;
          case 'CZ':
            n =
              /^(((00)([- ]?)|\+)(420)([- ]?))?((\d{3})([- ]?)){2}(\d{3})$/.test(
                d,
              );
            break;
          case 'DE':
            n =
              /^(((((((00|\+)49[ \-/]?)|0)[1-9][0-9]{1,4})[ \-/]?)|((((00|\+)49\()|\(0)[1-9][0-9]{1,4}\)[ \-/]?))[0-9]{1,7}([ \-/]?[0-9]{1,5})?)$/.test(
                d,
              );
            break;
          case 'DK':
            n = /^(\+45|0045|\(45\))?\s?[2-9](\s?\d){7}$/.test(d);
            break;
          case 'ES':
            n = /^(?:(?:(?:\+|00)34\D?))?(?:5|6|7|8|9)(?:\d\D?){8}$/.test(d);
            break;
          case 'FR':
            n =
              /^(?:(?:(?:\+|00)33[ ]?(?:\(0\)[ ]?)?)|0){1}[1-9]{1}([ .-]?)(?:\d{2}\1?){3}\d{2}$/.test(
                d,
              );
            break;
          case 'GB':
            n =
              /^\(?(?:(?:0(?:0|11)\)?[\s-]?\(?|\+)44\)?[\s-]?\(?(?:0\)?[\s-]?\(?)?|0)(?:\d{2}\)?[\s-]?\d{4}[\s-]?\d{4}|\d{3}\)?[\s-]?\d{3}[\s-]?\d{3,4}|\d{4}\)?[\s-]?(?:\d{5}|\d{3}[\s-]?\d{3})|\d{5}\)?[\s-]?\d{4,5}|8(?:00[\s-]?11[\s-]?11|45[\s-]?46[\s-]?4\d))(?:(?:[\s-]?(?:x|ext\.?\s?|#)\d+)?)$/.test(
                d,
              );
            break;
          case 'IN':
            n =
              /((\+?)((0[ -]+)*|(91 )*)(\d{12}|\d{10}))|\d{5}([- ]*)\d{6}/.test(
                d,
              );
            break;
          case 'MA':
            n =
              /^(?:(?:(?:\+|00)212[\s]?(?:[\s]?\(0\)[\s]?)?)|0){1}(?:5[\s.-]?[2-3]|6[\s.-]?[13-9]){1}[0-9]{1}(?:[\s.-]?\d{2}){3}$/.test(
                d,
              );
            break;
          case 'NL':
            n =
              /^((\+|00(\s|\s?-\s?)?)31(\s|\s?-\s?)?(\(0\)[-\s]?)?|0)[1-9]((\s|\s?-\s?)?[0-9])((\s|\s?-\s?)?[0-9])((\s|\s?-\s?)?[0-9])\s?[0-9]\s?[0-9]\s?[0-9]\s?[0-9]\s?[0-9]$/gm.test(
                d,
              );
            break;
          case 'PK':
            n = /^0?3[0-9]{2}[0-9]{7}$/.test(d);
            break;
          case 'RO':
            n =
              /^(\+4|)?(07[0-8]{1}[0-9]{1}|02[0-9]{2}|03[0-9]{2}){1}?(\s|\.|-)?([0-9]{3}(\s|\.|-|)){2}$/g.test(
                d,
              );
            break;
          case 'RU':
            n =
              /^((8|\+7|007)[-./ ]?)?([(/.]?\d{3}[)/.]?[-./ ]?)?[\d\-./ ]{7,10}$/g.test(
                d,
              );
            break;
          case 'SK':
            n =
              /^(((00)([- ]?)|\+)(421)([- ]?))?((\d{3})([- ]?)){2}(\d{3})$/.test(
                d,
              );
            break;
          case 'TH':
            n = /^0\(?([6|8-9]{2})*-([0-9]{3})*-([0-9]{4})$/.test(d);
            break;
          case 'VE':
            n =
              /^0(?:2(?:12|4[0-9]|5[1-9]|6[0-9]|7[0-8]|8[1-35-8]|9[1-5]|3[45789])|4(?:1[246]|2[46]))\d{7}$/.test(
                d,
              );
            break;
          default:
            n = /^(?:(1-?)|(\+1 ?))?\(?\d{3}\)?[-.\s]?\d{3}[-.\s]?\d{4}$/.test(
              d,
            );
        }
        return {
          message: t(
            e.l10n && e.l10n.phone
              ? a.message || e.l10n.phone.country
              : a.message,
            e.l10n && e.l10n.phone && e.l10n.phone.countries
              ? e.l10n.phone.countries[r]
              : r,
          ),
          valid: n,
        };
      },
    };
  };
});
